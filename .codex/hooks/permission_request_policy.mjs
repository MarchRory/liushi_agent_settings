...
import { allowPermissionRequest, denyPermissionRequest } from "./lib/hook-response.mjs";
import { loadSafetyPolicy } from "./lib/policy-loader.mjs";
import { commandMatchesAny, getDestructiveCommandPatterns } from "./lib/tool-classifier.mjs";
...
else if (isHighRiskRequest(command, text, destructivePatterns) && !hasApprovalRecord(text, policy.gates?.approval_record_required_fields ?? [])) {
...
function isHighRiskRequest(commandText, fullText, destructiveCommandPatterns) {
    return commandMatchesAny(commandText, destructiveCommandPatterns) || includesAny(fullText, highRiskSignals);
}
function hasApprovalRecord(textValue, fields) {
    const aliases = {
    ...
    return fields.every((field) => {
        ...
        return candidates.some((candidate) => includesAny(textValue, [candidate.replaceAll("_", " "), candidate]));
    });
}
